...
#######################################
# Add code to the districts and dates #
#######################################
...
# in the name of the file. So, I´m going to add this information to our studio. 
# I also add the District code, which is in the same order at the districts.

distritosPolicia <-distinct((select(df, DISTRITOS)), DISTRITOS)
codigo <- 1:24
CodigoDistrito <- data.frame(item1 = codigo, item2 = distritosPolicia)


Auxiliar <- read.csv("Auxiliar.csv")
df <- merge(x = df, y = Auxiliar, all.x = T, by.x = 'fichero.origen.integrado', by.y = "Fichero")
df <- merge(x = df, y = CodigoDistrito, all.x = T, by.x = 'DISTRITOS', by.y = "DISTRITOS")

# Sort the dataframe
df <- arrange(df, fecha)

names(df) <- c("DISTRITOS","Fichero", "RELACIONADAS_CON_LAS_PERSONAS", "RELACIONADAS_CON_EL_PATRIMONIO","POR_TENENCIA_DE_ARMAS","POR_TENENCIA_DE_DROGAS", "POR_CONSUMO_DE_DROGAS", "fecha", "codigo")
